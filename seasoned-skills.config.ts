...
export default defineConfig({
  ...
  additionalCriteria: [
    {
      text: 'A task is not done if it changed a command, a flag, a configuration key, the generated output, or the way of working itself without the documentation saying so in the same task: `docs/reference/` carries the names, and the pages above it carry the practice.',
      backedBy:
