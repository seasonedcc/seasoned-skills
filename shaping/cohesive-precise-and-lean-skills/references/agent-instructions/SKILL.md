...
The `seasoned-skills` package carries the skills and the CLAUDE.md content every project shares. A project adds its own facts in its workflow content files. Those are markdown files in the folder its configuration names. There is one per skill it extends, named after the skill, plus one for CLAUDE.md. The `seasoned-skills sync` command weaves both into the files agents load, one generated skill per skill and one generated CLAUDE.md. Nobody edits a generated file, because the next sync overwrites it. The sync also checks what it generates, under the limits in Size and shape below. The `seasoned-skills check` command runs the same checks, over the generated files or any file paths you give it. Part of it is Vale, a prose linter that runs our word rules.
...
7. **One home per rule.** A rule lives in exactly one place. Everywhere else that needs it points there. A skill points at CLAUDE.md the same way, since CLAUDE.md is always in front of the reader. Two copies of one rule drift apart the moment one is edited.
...
- A project's own facts live in that project's workflow content files. That includes workarounds for its tooling. A workaround goes stale the moment the project fixes what it worked around. The pull request with that fix is the one place the text can be deleted at the same time.
- A lesson from a task never lands in either home directly. It travels as an issue, on the project or on the package. The change to the text comes later, under the change bar. The self-improvement skill teaches how.
...
Any failing check fails the file. There are no warnings to weigh. When a sentence reads well and still fails, change it so the check passes, and say in the pull request that the rule may be the thing to fix.

## The change bar

Every change argues its case in the pull request that carries it, never in the instruction text. The text keeps a rule's scope and its reason, so a reader can apply it. The pull request keeps the case for changing it, so a reviewer can judge it. The case leaks into the text as a trace of what was removed, like "the list lives elsewhere, never here". Whoever reads the file after the merge never saw the old version. The sentence explains nothing to them.
...
The same bar covers every edit, addition or deletion. A lesson from a finished task, a slimming pass, a brand-new skill: all of them.
...
- A contradiction between a concrete recipe and a stated principle is a defect. The recipe is what an agent copies. So the recipe complies or the principle changes, never both.
