...
The `seasoned-skills` package carries the skills and the CLAUDE.md content every project shares. A project adds its own facts in its workflow content files. Those are markdown files: one for CLAUDE.md, plus one per skill it extends, named after the skill. The `contentDir` key in `seasoned-skills.config.ts` names their folder. The `seasoned-skills sync` command weaves the package content and the project files into the files agents load. That is one generated CLAUDE.md, and one generated skill per skill. A `triggers:` line in a project file's front matter adds words to the description the sync generates for that skill. Front matter is the block between the `---` lines. Never edit a generated file; the next sync overwrites it. The sync also checks what it generates, under the limits in Size and shape below. The `seasoned-skills check` command runs the same checks, over the generated files or any file paths you give it. The check runs Vale, a prose linter, for our word rules.

## Skill anatomy

A skill is a folder named after it, holding a `SKILL.md`. The package keeps its skills under `content/skills/`. A project commits its own under `.claude/skills/`, next to the generated ones. An agent decides whether to load a skill from its description alone. Once it does, the whole body loads.
...
- The body, everything below the front matter, carries what every use needs. Detail that only some uses need goes to a file under `references/`, loaded on demand. Helper programs go to `scripts/`, where no prose check reads them.
...
2. **Plain speech, no AI dialect.** Words that agents use with each other don't belong in instructions people review. The `Seasoned.Dialect` rule in Vale carries the list and names the plain word for each. The check prints both when it flags a word.
...
5. **Every rule carries its scope and its reason.** Say when it applies, when it doesn't, and why. The reason is what lets a reader handle a case you never saw coming. Without it, an agent applies a bare rule with confidence to the wrong case.

6. **Define terms on first meeting.** The instructions may teach a term of art, such as orchestrator, lane, or gate. They may also name a tool, such as Vale. The first time a reader meets either, a plain-words definition sits right beside it. If a tool is named before it is described, the reader guesses which tool.

7. **One home per rule.** A rule lives in exactly one place. Everywhere else that needs it points there. When the home is CLAUDE.md, point there too; every session loads it. Two copies of one rule drift apart the moment one is edited.
...
9. **Prefer tooling to prose.** A rule broken twice wants a mechanical guard, or wording that names the case it was broken in, never a louder paragraph. Once may be chance. Twice shows the prose isn't holding. A louder paragraph is more of the same prose. When a check can enforce the rule, build the check, and keep only the reason in prose. The checks live in the package, so building one is a pull request on the package.
...
12. **No sentence that reads wrong first.** "Instructions nobody reviews drift" sends the eye to a noun first. The reader has to back up to find the verb. Write "Instructions drift when nobody reviews them."

13. **Active voice where an agent acts.** "The tests should be run" hides who acts. Write "Run the tests."

## AI dialect

Sometimes a dialect word carries a concept the instructions really need. Keep the concept under a plain name, and let the word go.

Literal identifiers are the one exception, because a reader has to type or search for them as they are. Write command names, configuration keys, and exported symbols exactly as they are. Think `seasoned-skills sync` or `disable-model-invocation`. The concepts they carry still get plain names in prose: the sync, the switch that stops a skill loading on its own.

## Where a rule lives

A lesson has to land somewhere. A rule in the wrong home goes bad two ways. Authors restate it until the copies disagree, or it goes stale in a file nobody thinks to amend.
...
Every change argues its case in the pull request that carries it, never in the instruction text. That holds from a one-line lesson to a brand-new skill. The text keeps a rule's scope and its reason, so a reader can apply it. The pull request keeps the case for changing it, so a reviewer can judge it. When the case leaks into the text, it shows as a trace of what was removed, like "the list lives elsewhere, never here". Readers after the merge come to the file without the old version, so the sentence explains nothing to them.
...
When a change breaks a budget, make room honestly. Cut what fails the bar, move enforcement to tooling, or move detail to a reference file when only some uses need it. Never squeeze prose into dialect to fit — the budget counts words, but the point is what each word teaches.
...
- The author never reviews their own round of edits: they read the file as they meant it, not as it is. A fresh agent reviews as above. Fresh means this skill loaded and no memory of the edits. It gets the pull request and every touched file. It reports each sentence that breaks a principle, naming the principle. The author answers each finding in the pull request, fixing or declining it. The author never takes a finding at face value: a fresh reader knows the principles, not the history behind a sentence. Then a new fresh agent reads again. The cycle ends when the author is satisfied with the file, not when a reader runs out of findings. An empty round proves only that one reader found nothing.
- A contradiction between a concrete step, like a command to run, and a stated principle is a defect. The step is what an agent copies. So change the step to comply, or change the principle to match. Leaving both standing keeps the defect.
