...
- The author never reviews their own round of edits: they read the file as they meant it, not as it is. A fresh agent, with this skill loaded and no memory of the edits, reads the pull request and every touched file whole. It reads as someone about to use the file, say to record a lesson or to write a skill. It reports where the file fails that use: a rule it couldn't apply, a sentence read wrong, a thing missing, a thing not needed. It doesn't grade sentences against this file's rules one by one. A reader in that mode wants every term defined and every tool named. The file then grows past its budget on findings that were each right alone. A finding that asks for words names the home they belong to and the words it would cut to pay for them. The author answers each finding in the pull request under the change bar, like any other change, and declines what fails it. Then a new fresh agent reads. The cycle ends when the author is satisfied with the file, not when a reader runs out of findings.
