...
import { CueNotFoundError, findCue, progressAt, scheduleMoves } from './cues'
import type { Move } from './screenplay'

describe('findCue', () => {
  it('finds a cue that the screenplay wrapped across lines', () => {
    const narration = 'Here is the recipe\n  list inside Avatar Foods.'
...
describe('scheduleMoves', () => {
  ...
  it('lets a hard offset win over anything the words say', () => {
    const moves: Move[] = [
      { do: 'hold', seconds: 1, atSecond: 4.5, cue: 'five' },
    ]

    expect(scheduleMoves('scene-01', narration, 20, moves)[0].startsAt).toBe(
      4.5
    )
  })
  ...
  it('leaves a move with no sync hint to follow the one before it', () => {
    ...
    expect(
      scheduleMoves('scene-01', narration, 20, moves)[0].startsAt
    ).toBeNull()
  })
