---
name: post-review
description: Post a completed code review on a teammate's GitHub PR as inline comments anchored to the diff, with the verdict and blocking rationale in the review summary. Use when posting review findings to GitHub, requesting changes or approving a PR, taking over a PR by moacirmoda, or after analyzing a PR (the pr-review skill covers the analysis) when the findings should land on the PR itself.
---
...
## Moacir's PRs: take over instead of posting a review

When the PR's author is moacirmoda, nothing above applies as written. Moacir contributes PRs in place of feature requests and does not return to work review feedback; by agreement, the user takes his PRs over. Never post a review asking him to change anything, and never open follow-up PRs for him to accept — the resolutions land on his PR directly:
...
- When the branch is merge-ready: rewrite the PR description to current truth for an external reader, preserving Moacir's original spec block (it is the intent record); reply on every open thread with its resolution; resolve the threads; and dismiss any now-stale blocking review, crediting what got fixed.
