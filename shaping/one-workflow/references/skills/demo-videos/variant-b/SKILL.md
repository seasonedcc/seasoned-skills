...
The machinery is built and documented. Read [`scripts/demo-videos/rig/README.md`](../../../scripts/demo-videos/rig/README.md) for the screenplay format, the moves, how sync is measured and how retakes work, and [`scripts/demo-videos/README.md`](../../../scripts/demo-videos/README.md) for the narrator, the deliveries and the transcribe check. This skill is the part that is not machinery: how a scope becomes a screenplay, how a screenplay sounds, and what has to be true before a video is handed to anyone.

Everything runs on Apple Silicon with `ffmpeg` on `PATH`. Once per machine:

```bash
scripts/demo-videos/setup.sh
```
...
- **The docs coverage manifest**, `app/content/docs/coverage.ts`, plus the articles the project added under `app/content/docs`. A surface that shipped documented is a surface with a walkthrough already written for it.
- **The document itself.**
...
Two more that follow from the same place: never narrate something the video does not show — the old world a contrast reaches back to is the one exception, and it stays a sentence on the narration track — and never narrate a screen the viewer is not looking at yet. The [`copywriting`](../copywriting/SKILL.md) skill governs how we write product words; load it when you need a term the product itself would use.

## 4. Write the screenplays

A screenplay is a TypeScript module under `demo-videos/<slug>/`, named by its cut: `highlights.ts` and `full.ts`. The types are in [`scripts/demo-videos/rig/screenplay.ts`](../../../scripts/demo-videos/rig/screenplay.ts) and the format is documented in the rig README. Read both before writing one.
...
Film against a **fresh worktree with a freshly seeded database** — `pnpm run worktree:setup <lane>`, then serve the built product from that same worktree. The rig reads `DATABASE_URL` from the worktree's `.env` to mint the actor's session, so the rig and the server must be the same checkout pointed at the same database.

```bash
pnpm run build
PORT=<worktree port> NODE_ENV=test node ./server.js
```

`NODE_ENV=test` is how this repo serves the real build over plain HTTP. Under `production` the canonical-URL middleware redirects to HTTPS and the browser cannot connect. A dev server also works, but Vite compiles a route the first time it is asked for and that wait lands in the middle of a take — list every route a scene walks into under its `warm` if you film one.

**Every row a scene needs comes from `pnpm run db:seed:dev` on an empty database.** Never hand-craft demo state: it cannot be reproduced, so the video cannot be re-rendered. If a scene wants state the seed does not have, the seed is what to change — and a seed change ships under the normal rule, with its coverage in the seed manifest suite. The seed is single-shot and refuses a populated database; to reseed, drop the database and build it again.
...
`demo-videos/<slug>/DEMO-STATE.md` names the product commit, the database recipe, the actor (`userEmail` and `companyName`, both of which must exist in that database), what each scene needs, and the server command. `demo-videos/rig-sample/DEMO-STATE.md` is the shape to copy.

## 6. Film it

```bash
PORT=<port> pnpm run demo:video <slug> highlights
PORT=<port> pnpm run demo:video <slug> full
```

The rig speaks each scene, warms its routes, performs the choreography at human pace, finds the frame the clapper ends on, cuts the footage to the narration and assembles the cut. Everything it makes lands under `scripts/demo-videos/out/<slug>/<cut>/`, and every assembly also copies the finished video beside its screenplay, as `demo-videos/<slug>/<YYYY-MM-DD>-<slug>-<cut>.mp4` — `demo-videos/recipe-dna-moves-in/2026-08-18-recipe-dna-moves-in-highlights.mp4`. That copy is the one to hand over.

Retakes, all documented in the rig README:

```bash
pnpm run demo:video <slug> <cut> --scene scene-04-costing   # re-film one scene, reassemble
pnpm run demo:video <slug> <cut> --assemble                 # reassemble what is on disk
pnpm run demo:video <slug> <cut> --refresh-narration        # speak every scene again
pnpm run demo:video <slug> <cut> --base-url http://…        # film a different server
```
...
```bash
pnpm run demo:video <slug> <cut> --check
```
...
**2. Read the frames as images.** `scripts/demo-videos/out/<slug>/<cut>/frames/*.jpg`, one every four seconds. Actually look at them: the cursor visible and on what the screenplay put it on, the product real and fully rendered, no half-loaded list, no stray overlay, nothing on screen that should not be in a video. A five-minute cut is about seventy frames and you read all of them; a forty-minute cut is six hundred, so read a sample spread evenly across it plus every frame of any scene the check or the drift already flagged.
...
scripts/demo-videos/transcribe.sh --audio /tmp/slice.wav
...
`demo-videos/rig-sample/` is a **test card, not a template**. It says so in its own first sentence. Copy its file layout; copy nothing about its narrative, because a real cut opens on the struggling moment its shaping document names, not on a statement about itself.
...
One PR per video project, built in an isolated worktree. Gates: `pnpm run lint`, `pnpm run tsc`, `pnpm run test:unit` — which carries the rig's TypeScript tests — plus the narration comparator suite `scripts/demo-videos/test.sh`, and the self-review above passing on both cuts. `test.sh` runs on the checkout's `scripts/demo-videos/.venv`, which is gitignored and therefore per-checkout: from `scripts/demo-videos/`, `uv venv --python 3.12 .venv` then `uv pip install --requirement requirements.txt` builds it without the ~3 GB of model weights the comparator never loads (`setup.sh` also builds it, along with the weights). The product Definition of Done items — MCP parity, a docs article, a dev-seed section, E2E coverage — do not apply to a video project, which is repo content rather than a product surface. A seed change made *for* a video is a product change, and it ships under the full rule with its coverage in the seed manifest suite.
