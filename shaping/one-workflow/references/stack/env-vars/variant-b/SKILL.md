...
So a newly required variable has to reach every environment that boots the server before the code requiring it runs there: `.env`, `.env.test`, the CI workflow, and the DigitalOcean app settings.
...
4. Add the value to the production app settings on DigitalOcean before the release ships — the `deploying` skill covers where
