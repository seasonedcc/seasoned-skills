...
Validation errors for array fields come back nested by position, not flat: `{ tries: { "0": ["Informe um número"], "1": ["…"] } }`. Positions index the submitted array, so when the client compacts gaps before submitting, map each position back through the submitted slots to anchor the error on the right input.
...
The convention across the codebase: `<name>FormSchema` — only the fields the form actually renders — for the route's `act()`/`SchemaForm`, and a `<name>Schema` — form fields plus the ids — for the business function's `applySchema()`. The action supplies the ids from `params`.

`act()` builds the mutation input as `{ ...search, ...params, ...values }` (`app/framework/controllers/act.server.ts`), so the parsed form values win last: they override any route param or search param of the same name, including coerced defaults for fields the form never submitted (an unchecked checkbox coerced to `false`, an absent text field coerced to `''`). Keep route-param-named fields out of the form schema — a schema field sharing a name with a route or search param silently clobbers the param with its parsed/coerced value. Extend `app/framework/controllers/act.server.test.ts` when you change how `act()` resolves input.
...
- Every guard/validation `InputError` needs an explicit field path to surface as an inline field error — a pathless error (or one under a key the form doesn't render) produces no visible field-level feedback.
