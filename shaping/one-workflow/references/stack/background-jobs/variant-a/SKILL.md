...
Background jobs use graphile-worker via `makeJob` and `makeCronJob` from `~/framework/worker.server`. The app registers its jobs in `apps/web/app/business/jobs.server.ts` and runs them in a separate process (`run-worker.ts`).
...
Every job (both `makeJob` and `makeCronJob`) must be added to the `jobs` array in `apps/web/app/business/jobs.server.ts`:
...
await db()
  ...
  .execute(async (trx) => {
    ...
    await trx.updateTable('items').set({ status: 'done' }).where('id', '=', itemId).execute()
  })
```
...
- Framework infrastructure: `apps/web/app/framework/worker.server.ts`
- Job registration: `apps/web/app/business/jobs.server.ts`
- Worker entry point: `run-worker.ts`
- On-demand job example: `apps/web/app/business/auth.server.tsx` (`sendAuthEmail`)
- Cron job example: `apps/web/app/business/results.server.ts` (`enqueuePendingResults`)
- Fan-out pattern example: `apps/web/app/business/results.server.ts`
- Test examples: `apps/web/app/business/results.server.test.ts`, `apps/web/app/business/auth.server.test.ts`
