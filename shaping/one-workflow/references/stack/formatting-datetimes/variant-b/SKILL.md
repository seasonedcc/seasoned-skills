...
### Company-local display

User-facing labels on company-local surfaces (receiving, production runs, staff scheduling) are formatted at the company's configured time zone, WITHOUT the `TZ` suffix — the company time zone is implied:

```typescript
import { dateInCompanyTimezone, todayInCompanyTimezone } from '~/business/company-timezones.common'

sql<string>`to_char(scheduled_at at time zone ${sql.lit(currentCompany.timezone)}, 'HH24:MI')`.as('scheduledTime')
```

Output: `14:30`

The time zone is per-company configuration, stored append-only in `companyTimezoneRevisions` (default `America/Los_Angeles`). `app/business/company-timezones.common.ts` is the single home of the helpers (`dateInCompanyTimezone(timezone, instant)`, `todayInCompanyTimezone(timezone)`, `DEFAULT_COMPANY_TIMEZONE`, `companyTimezoneSchema`). They take the time zone string as their first argument — never hardcode a zone.

- **Context-bearing business functions** read `currentCompany.timezone` (populated on the auth context) and pass it into the helpers.
- **Seed and job paths** without the auth context call `currentCompanyTimezone(executor, companyId)` from `~/business/companies.server` to resolve the string first.

The time zone is user-configurable and interpolated via `sql.lit`, so every write path validates it against `companyTimezoneSchema` — a strict whitelist backed by `Intl.supportedValuesOf('timeZone')`. Routes carry no time zone knowledge beyond threading `currentCompany.timezone`.
