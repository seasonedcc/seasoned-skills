...
Then ask a third: did the effort change a mechanism, command, or file that a skill in `.claude/skills` describes? Nothing type-checks a skill and no test reads one, so a skill's description becomes false the moment its mechanism changes, and a stale skill misleads future agents more confidently than no skill at all. Grep the skills for whatever the effort touched, and true up every statement the work made false before the pass closes. When the Definition of Done has grown since the calibrations in `workflow-content/calibrations.md` were measured, its numbers understate what a stage now costs — recalibrate them in this pass from the effort's own measured context windows.
...
1. **A pull request on the project.** Solve the problem locally: land the lesson in the affected file — the project's own committed material, or, for a lesson about a generated skill, that skill's content file under `workflow-content/`. Never edit a generated skill itself: the generated text is rebuilt on every upgrade, and the edit dies with the next regeneration.
...
Project-empirical lessons about this skill land in `workflow-content/self-improvement.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
