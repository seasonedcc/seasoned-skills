...
## The release: a deployed product

A release is a GitHub Release tagged `v<N>` against a commit on main, and publishing it is what deploys production — the project's own deploy documentation owns the pipeline mechanics; read it before starting. This practice owns everything that happens before the publish button: what the release says, how the requests-from-meetings records learn that their asks shipped, and the audit that gates the whole thing.

### Scope the release

- The next tag is the highest existing `v<N>` plus one: `git tag -l | sort -V`.
- The span is `v<last>..origin/main`. Enumerate every merged PR in it — `git log v<last>..origin/main --first-parent --oneline` for the merge commits, then `gh pr view <n>` for each — and study the span as the spine demands: the notes below are only as good as this reading.
...
1. **Opening** — a few sentences on what this release means for the people using the product. Plain product language, no internal vocabulary.
...
Write for an external reader, in the product's own words. The curated part must be considerably richer than the auto-generated notes — if a section reads like the PR-title list below it, rewrite it.
...
Merge only when the audit and CI both pass, and hold the publish until the user confirms the settings are in place.

### Publish

Asking for a release is the authorization to publish — publishing deploys production, and the user knew that when they asked; never ask for a further go. Once the pre-release PR has merged with the audit and CI both passing:
...
Then verify: the release page shows the curated notes first and the auto-generated notes last, the badge links resolve to the release, and the production deploy workflow run completes green (`gh run watch`).

A green run is not a working site. The release is not done until the project's post-deploy smoke test has run against the production URL and come back clean.

## Where lessons go

Project-empirical lessons about this skill land in `workflow-content/release.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
