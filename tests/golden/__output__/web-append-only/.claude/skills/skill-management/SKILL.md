...
description: Create, manage, and debug Claude Code Agent Skills. Use when creating new skills, debugging skill activation issues, writing SKILL.md files, managing skill structure, or learning about Claude Code skills. Helps with personal skills, project skills, YAML frontmatter, descriptions, and troubleshooting. Also use when someone asks to "wire up a helper skill" or where the operational playbooks live.
---
...
Skills in this repository live only at the project level — never create personal skills for team conventions.

## Where lessons go

Project-empirical lessons about this skill land in `workflow-content/skill-management.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
