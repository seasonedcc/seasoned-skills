...
1. **Page routes** export ErrorBoundary — whether they are leaf routes (e.g., `profile.tsx`) or routes with child drawers/modals (e.g., `items/index.tsx`, `reports/index.tsx`). If it renders its own page content, it gets an ErrorBoundary.
...
Project-empirical lessons about this skill land in `workflow-content/nested-routes.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
