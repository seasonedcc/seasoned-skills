...
One PR per meeting, built in an isolated worktree provisioned by the workflow's provisioning, skipping the seed — the gates need the provisioned environment a plain `git worktree add` does not give you, and a content lane has no use for a seeded database. Branch `worktree/meeting-YYYY-MM-DD`; PR title like "Record feature requests from the 2026-07-27 Software Review meeting", body written for an external reader, with the verifier's summary line in it. Gates: `pnpm run lint`, `pnpm run tsc`, `pnpm run test:unit`, a clean verifier run, and a clean audit with its findings fixed (see *Audit before shipping*). The Definition of Done's product-surface criteria do not apply — this is repository content, not a product surface, exactly the doctrine's documentation-only carve-out.
