...
This loop is the workflow's review loop, whole. The pre-merge adversarial review runs it against the draft pull request — reviewing the draft is deliberate, because the loop reads the PR's description and discussion alongside the diff, which a bare diff review never sees — and the PR flips from draft to ready for review only after the loop has run and every confirmed finding carries a ruling. Small diffs are not exempt from the loop; they simply overlap the rest of the pipeline.
...
**Conventions.** Check the diff against the rules that govern the changed code: the repo's own `AGENTS.md`/`CLAUDE.md` and the binding process docs it commits. Flag a violation only when you can quote the exact rule and the exact line that breaks it — no style preferences, no "spirit of the doc" inferences.
...
Project-empirical lessons about this skill land in `workflow-content/pr-review.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
