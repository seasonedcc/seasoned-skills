...
Project-empirical lessons about this skill land in `workflow-content/optimistic-ui.md` through a pull request on the project — never by editing this file, which is regenerated on every upgrade. A lesson that turns out to be true of every project travels as an issue on the workflow package instead.
