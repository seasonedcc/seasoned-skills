...
import { travelPath, typingCadence } from '../../../runtime/demo-videos/rig/pacing.js'
import { HUMAN_PACING } from '../../../runtime/demo-videos/rig/screenplay.js'

const from = { x: 100, y: 100 }
const to = { x: 700, y: 400 }

/** Index access under noUncheckedIndexedAccess: a missing step is a test
 *  failure, not an undefined to reason about. */
...
describe('travelPath', () => {
  ...
  it('takes the distance divided by the speed, inside its own limits', () => {
    ...
    expect(seconds).toBeCloseTo(distance / HUMAN_PACING.travel.pixelsPerSecond, 5)
  })

  it('never crosses a screen faster than its floor', () => {
    const seconds = travelPath({ x: 0, y: 0 }, { x: 12, y: 0 }, HUMAN_PACING, 1).reduce(
      (total, step) => total + step.after,
      0,
    )
  ...
  it('bows away from the straight line rather than running down it', () => {
    const path = travelPath(from, to, HUMAN_PACING, 1)
    const middle = at(path, Math.floor(path.length / 2) - 1)
    const straight = {
      x: (from.x + to.x) / 2,
      y: (from.y + to.y) / 2,
    }
  ...
  it('eases: it covers less ground in its first tenth than in its middle', () => {
    ...
    const step = (index: number) =>
      Math.hypot(
        at(path, index).x - at(path, index - 1).x,
        at(path, index).y - at(path, index - 1).y,
      )
...
describe('typingCadence', () => {
  ...
  it('pauses longer at a space than inside a word', () => {
    ...
    expect(at(strokes, 2).character).toBe(' ')
    expect(at(strokes, 2).after).toBeGreaterThan(at(strokes, 1).after)
  })

  it('pauses longer still at a full stop', () => {
    const strokes = typingCadence('a. b', HUMAN_PACING)

    expect(at(strokes, 1).after).toBeGreaterThan(at(strokes, 2).after)
  })
