...
import { takeFingerprint } from '../../../runtime/demo-videos/rig/narration.js'
import {
  defineProduction,
  HUMAN_PACING,
  type Production,
  type Scene,
} from '../../../runtime/demo-videos/rig/screenplay.js'
...
const scene: Scene = {
  ...
  narration: 'Here is the recipe list inside Acme Foods.',
  needs: [],
  open: { path: '/app/production/recipes' },
  choreography: [],
}

const fingerprint = (overrides: Partial<Scene> = {}, spoken: Production = production) =>
...
describe('takeFingerprint', () => {
  ...
  it('changes when the line changes', () => {
    expect(fingerprint({ narration: 'Here is the recipe list.' })).not.toBe(fingerprint())
  })
  ...
  it('ignores what a scene does not affect', () => {
    expect(fingerprint({ beat: 'Something else entirely', tail: 4 })).toBe(fingerprint())
  })
