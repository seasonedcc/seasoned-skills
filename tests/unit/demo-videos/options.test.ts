...
import { parseArguments } from '../../../runtime/demo-videos/rig/options.js'
...
describe('parseArguments', () => {
  ...
  it('does not mistake an option value for the cut', () => {
    const options = parseArguments(['rig-sample', '--scene', 'scene-02-search', 'proof'])
  ...
  it('refuses an option whose value was swallowed by the next option', () => {
    expect(() => parseArguments(['rig-sample', 'proof', '--scene', '--check'])).toThrow(
      '--scene needs a value',
    )
  })
