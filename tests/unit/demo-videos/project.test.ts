...
import {
  ...
  handOver,
  handOverPath,
} from '../../../runtime/demo-videos/rig/project.js'

async function finishedCut(rendered: string): Promise<FinishedCut> {
  const root = await mkdtemp(path.join(tmpdir(), 'demo-video-'))
  const source = path.join(root, 'demo-reel-moves-in')
  const video = path.join(root, 'out', 'demo-reel-moves-in-highlights.mp4')
  await mkdir(source)
  await mkdir(path.dirname(video))
  await writeFile(video, rendered, 'utf8')
  return { slug: 'demo-reel-moves-in', cut: 'highlights', source, video }
}
...
describe('handOverPath', () => {
  it('names the video by the day it was assembled and the whole project', () => {
    const finished = {
      slug: 'demo-reel-moves-in',
      cut: 'highlights',
      source: 'demo-videos/demo-reel-moves-in',
      video:
        '.claude/skills/demo-videos/scripts/out/demo-reel-moves-in/highlights/demo-reel-moves-in-highlights.mp4',
    }

    expect(handOverPath(finished, new Date(2026, 7, 18, 23, 45))).toBe(
      'demo-videos/demo-reel-moves-in/2026-08-18-demo-reel-moves-in-highlights.mp4',
    )
  })
})
...
describe('handOver', () => {
  it('copies the finished video beside the screenplay it was filmed from', async () => {
    ...
    expect(await beside(finished)).toEqual([
      '2026-08-18-demo-reel-moves-in-highlights.mp4',
    ])
  })
  ...
  it('keeps the video of every earlier day it was rendered on', async () => {
    ...
    expect(await beside(finished)).toEqual([
      '2026-08-18-demo-reel-moves-in-highlights.mp4',
      '2026-09-02-demo-reel-moves-in-highlights.mp4',
    ])
    expect(
      await readFile(
        path.join(finished.source, '2026-08-18-demo-reel-moves-in-highlights.mp4'),
        'utf8',
      ),
