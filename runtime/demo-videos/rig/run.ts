...
import { listenBack, readTake, speak, takeFingerprint } from './narration.js'
import { parseArguments } from './options.js'
import { type SceneTake, record } from './perform.js'
import { PROJECTS_ROOT, type Project, handOver, loadProject } from './project.js'
import type { Scene } from './screenplay.js'
import { closeSession, sessionCookie } from './session.js'

// The project's session module reads its configuration from the environment,
// the way the product does. Load the project's .env for it when there is one.
try {
  process.loadEnvFile(path.resolve(PROJECTS_ROOT, '..', '.env'))
} catch {
  // No .env at the project root — the environment is already whatever it is.
}
