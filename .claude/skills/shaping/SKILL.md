---
name: shaping
description: Run framing and shaping sessions that end in a shaping document, and answer questions about the method from the verbatim books, posts, and talk transcripts in references/. Use when framing a raw idea, shaping a project, writing or revising a shaping document under shaping/, working feedback on one back in, breadboarding or drawing fat marker sketches, finding and defusing time bombs (Shape Up's rabbit holes), naming no-gos, or when reasoning about customer demand — jobs to be done, JTBD, the struggling moment, forces of progress, demand-side selling, why people switch. Also the corpus to consult on anything Shape Up covers beyond our process — pitches, appetite, betting tables, scopes, hill charts, scope hammering, deciding when to stop.
---

# Shaping

This skill runs framing and shaping sessions: a human brings a candidate, and the work ends in one document a whole company can read and a build can start from. It also carries the verbatim text of the books, posts, and talks the method comes from, at `references/`. `references/INDEX.md` lists every document and where it came from.
...
2. If the repo keeps standing principles at `shaping/principles.md`, read them too: they are rulings that bind every session and every document in that repo.
...
**The relentless interviewer.** One question at a time, in plain conversation — never a questionnaire or a form widget, because adjudication is a dialogue, not a survey — each carrying your recommended answer. Explore the materials before asking — the demand records, the codebase, the sibling documents — so no question is one the room could have answered for you. Keep going until you and the user reach shared understanding, walking each branch of the decision tree.
...
2. **Read.** Open the matching documents in full. For long ones (>250 lines), read by offset — the opening first, then the targeted sections. Transcripts in `04-videos/` and `05-lenny-interview/` put a whole paragraph on each line, so a Grep hit there returns a very long line: read the document around the match's `[12:34]` timestamp instead of working from the Grep output alone. Some prose exists only inside images — the book embeds whole pitches as screenshots (the To-Do Groups pitch lives in `01-shape-up/images/1.5-to-do_groups_pitch.png`, not in any markdown) — so a quote that greps to nothing may still be real: open the chapter's images before concluding the words aren't in the corpus.
...
- `references/04-videos/` — talks, live streams, and podcast episodes from Ryan Singer's channel and 37signals', one file per video: the method explained out loud, and teams walked through applying it.
- `references/05-lenny-interview/` — Lenny Rachitsky's 2025 interview with Ryan Singer: the full transcript, beside the free portion of the newsletter post that published it.
...
Some transcripts carry still frames of what was on screen, each an image followed by an italic caption at its own timestamp. The caption describes the picture and was written for this corpus — it is not something anyone said, so read it as a description and never quote it as the source's words.

## Installing into a project

Shaping documents live in the project they are about, not in this skill. The first time this skill runs in a repository, create `shaping/` and copy the assets out of the installed skill:

```sh
mkdir -p shaping
cp -R .claude/skills/shaping/shaping/assets shaping/assets
```

Then start each document by copying the template into a folder named after the project:
...
A repo can also keep `shaping/principles.md`: standing rulings that bind every session run in that repo — a house doctrine the skill itself stays free of. Sessions read it as part of their opening ritual, so anything a repo's teams have settled once holds everywhere without being restated.
